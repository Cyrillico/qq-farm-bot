...
function updateFriendRuntimeSettings(patch = {}) {
    friendRuntimeSettings = {
        ...friendRuntimeSettings,
        ...patch,
    };
...
function getFriendRuntimeSettings() {
    return { ...friendRuntimeSettings };
}
...
async function runManualFriendOpCore({ gid, action }) {
    ...
    const isDangerousAction = pickedAction === 'putBug' || pickedAction === 'putWeed' || pickedAction === 'bad';
    if (isDangerousAction && !friendRuntimeSettings.enablePutBadThings) {
        throw new Error('dangerous friend actions disabled');
    }
    ...
    try {
        ...
        for (const f of friends) {
            ...
            if (hasSteal) {
                ...
                visitedGids.add(gid);
            } else if (friendRuntimeSettings.enablePutBadThings && canPutBugOrWeed) {
